...
async function runMigration() {
  try {
    ...
    await client.query('ALTER TABLE vehicles_cache ENABLE ROW LEVEL SECURITY');
    await client.query('CREATE POLICY "vehicles_cache_public_read" ON vehicles_cache FOR SELECT USING (true)');
    await client.query(`
    ...
    await client.query('ALTER TABLE interested_vehicles ENABLE ROW LEVEL SECURITY');
    await client.query(`
    ...
    `);
    await client.query('CREATE POLICY "interested_vehicles_user_insert" ON interested_vehicles FOR INSERT WITH CHECK (auth.uid() = user_id)');
    await client.query(`
    ...
    await client.query('ALTER TABLE search_queries ENABLE ROW LEVEL SECURITY');
    await client.query('CREATE POLICY "search_queries_user_isolation" ON search_queries FOR SELECT USING (auth.uid() = user_id)');
    await client.query('CREATE POLICY "search_queries_user_insert" ON search_queries FOR INSERT WITH CHECK (auth.uid() = user_id)');

    await client.query('ALTER TABLE vehicle_validations ENABLE ROW LEVEL SECURITY');
    await client.query(`
